...
save_solution = True
Wsol_filename = "W_sol_OCB"+str(d)+"D"
TAsol_filename = "TA_sol_OCB"+str(d)+"D"
TBsol_filename = "TB_sol_OCB"+str(d)+"D"
...
## Define seesaw algorithm
def see_saw_step(num_iter):
    TAs = gen_rand_oper(B, nIN, d, ns)
    TBs = gen_rand_oper(B, 2*nIN, d, ns)

    
    for _ in range(num_iter):
        Ws = see_sawW(BW, TAs, TBs, n, d, ine='OCB')
        TAs = see_sawT_OCB(B, Ws, TBs, 'TA', ns, d, ine='OCB')
        TBs = see_sawT_OCB(B, Ws, TAs, 'TB', ns, d, ine='OCB')


    GY = OCB_expr(TAs, TBs, 'W')
    obj_val = Ws.T @ GY
...
print("Optimzing OCB inequality for boxworld processes of dim", d)
...
if verify_solution:
    ...
    for y in range(2*nIN):
            print("Conditions for TB[%d]:" %y, check_oper_const(TBsol[y]+TBsol[y+4], symexpr, d, ns, verbose=True))
            print("Normalization of TB[%d]:" %y, sum(TBsol[y]+TBsol[y+4]))
...
if save_solution:
    ...
    head = "# Each row represents T[b,y,y'] (ordering 4*b+2*y+y'), label of spaces as O, O', I, I'"
